 ...
 *
 * The whole surface used here -- RegisterPayloadHandlersEvent, PayloadRegistrar (.versioned/
 * .playToServer/.playToClient), IPayloadContext.player()/enqueueWork, PacketDistributor
 * .sendToPlayer, and connection.hasChannel(Type) -- is identical on NeoForge 20.6..26 (verified
 * against the cached universal jars, 2026-07-05), so the only cog block is a range guard on the
 * registrar: a cell below 1.20.5 (IPayloadRegistrar era) fails generation instead of compiling
...
public final class UltimateSleepNet {
    ...
    public static void registerPayloads(RegisterPayloadHandlersEvent event) {
        ...
        //compat_loaders.check_range(ver)
        //cog.outl('        PayloadRegistrar registrar = event.registrar(UltimateSleep.MOD_ID).versioned("1");')
        //]]]
