...
/**
 * The Ultimate Sleep admin panel (pre-26 GuiGraphics pipeline): paginated (8 setting pages + a
 * Sleep-Admin page), reading values from {@link ClientState} (server-synced) and writing over the
 * back-channel; the server validates permissions and re-syncs. Behavior-identical twin of the 26.x
 * extractRenderState version in shared_minecraft. Era drift handled by cog: mouseScrolled arity
 * (4 doubles from 1.20.4) and EditBox.setCentered (1.21.8+).
 */
public final class UltimateSleepScreen extends Screen {
    ...
    private void buildSettingRow(String key, int labelX, int ctrlX, int y, int ctrlW) {
        ...
        if (ClientState.isBool(key)) {
            ...
            //if compat.centered(ver): cog.outl("            eb.setCentered(true);")
            //]]]
            eb.setCentered(true);
            //[[[end]]]
    ...
    private void buildAdminPage(int left, int rowTop, int ctrlW) {
        ...
        //[[[cog
        //if compat.centered(ver): cog.outl("        addAdminField.setCentered(true);")
        //]]]
    ...
    //[[[cog
    //if compat.scroll4(ver):
    //    cog.outl('    @Override')
    //    cog.outl('    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {')
    //    cog.outl('        if (page == ADMIN_PAGE && ClientState.admins.size() > MAX_ADMIN_ROWS) {')
    //    cog.outl('            int maxScroll = ClientState.admins.size() - MAX_ADMIN_ROWS;')
    //    cog.outl('            adminScroll = Math.max(0, Math.min(maxScroll, adminScroll - (int) Math.signum(scrollY)));')
    //    cog.outl('            rebuildWidgets();')
    ...
    //else:
    //    cog.outl('    @Override')
    //    cog.outl('    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {')
    //    cog.outl('        if (page == ADMIN_PAGE && ClientState.admins.size() > MAX_ADMIN_ROWS) {')
    //    cog.outl('            int maxScroll = ClientState.admins.size() - MAX_ADMIN_ROWS;')
    //    cog.outl('            adminScroll = Math.max(0, Math.min(maxScroll, adminScroll - (int) Math.signum(delta)));')
    //    cog.outl('            rebuildWidgets();')
    //    cog.outl('            return true;')
    //    cog.outl('        }')
    //    cog.outl('        return super.mouseScrolled(mouseX, mouseY, delta);')
    //    cog.outl('    }')
    //]]]
    @Override
